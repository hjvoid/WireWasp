...
export async function scanForm(url: string, action: string, method: 'GET' | 'POST', inputs: string[]): Promise<[string, string, string] | null> {
    const formURL = new URL(action, url).href // Handle relative form actions

    console.log(`🔍 Scanning form at ${formURL} (${method}) with fields: ${inputs.join(', ')}`)  
